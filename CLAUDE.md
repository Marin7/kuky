...
- **Homework formats**: `MANUAL` (free-text, `PENDING→SUBMITTED→REVIEWED`) and `EXERCISE` (auto-graded, `SINGLE_CHOICE`/`MULTI_CHOICE`/`TRUE_FALSE`/`MULTI_BLANK`/`DRAG_DROP`/`TABLE_FILL`/`MATCHING`, terminal `GRADED` status). Answer key hidden from students pre-submit via separate DTOs.
...
`specs/029-presentation-activities/plan.md`
<!-- SPECKIT END -->
